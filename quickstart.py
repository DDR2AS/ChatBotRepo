...
# Creación del evento
def CreateEvent(identificator, service, strdateappointment, summary, description=None, duration=0.5,
    ...
    if len(matches):
        ...
        service.events().insert(calendarId=identificator, body=event).execute()
        return event
...
def VerificarIdentidad(dni):
    ...
    token = "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9" \
            ".eyJ1c2VyX2lkIjoxNzE3LCJjb3JyZW8iOiJkaWVnby5hdmlsYTEyMzk0QGdtYWlsLmNvbSIsImlhdCI6MTY2MDQwMDYxNX0" \
            ".lRhe0yQd2LdxUlKA_o4FNVj3TCTnnNvpLClCVEzgZLw "
...
def consultNameOwner(dni):
    ...
    if dataJson['success']:
        strName = dataJson['result']['nombres'] + " " + dataJson['result']['paterno'] + " " + dataJson['result'][
            'materno']
